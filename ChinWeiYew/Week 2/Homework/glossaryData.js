...
let table = document.querySelector('#glossary')
...
inputSearch.addEventListener("keyup", searchDefinition)
function searchDefinition() {
  dropdown.value = ''
  uncheck()
  let inputSearch = document.querySelector('#inputSearch').value.toLowerCase()
  let result = glossary.filter(x => x.term.toLowerCase().indexOf(inputSearch) >= 0)
  table.innerHTML = ''
  firstTR.innerHTML = ''
  resetTable(result)
}
...
let classList = []
glossary.forEach((x) => {
  if (classList.indexOf(x.class) < 0) {
    let option = document.createElement("option")
    option.value = x.class
    dropdown.appendChild(option)
    option.innerHTML = `${x.class}`
    classList.push(x.class)
  }
})

dropdown.addEventListener("change", searchClass)
function searchClass() {
  inputSearch.value = ''
  uncheck()
  let selectClass = document.querySelector('select').value
  let result = glossary.filter(x => x.class == selectClass)
  table.innerHTML = ''
  firstTR.innerHTML = ''
  resetTable(result)
}
...
glossary.forEach((x) => {
  for (let i=0; i< x.tags.length; i++) {
    if (tagList.indexOf(x.tags[i]) < 0) {
      let checkboxInput = document.createElement("input")
      checkboxInput.value = x.tags[i]
      checkboxInput.type = 'checkbox'
      checkboxInput.name = 'tags'
      checkbox.appendChild(checkboxInput)
      let labelCheckbox = document.createElement("label")
      checkbox.appendChild(labelCheckbox)
      labelCheckbox.innerHTML = `${x.tags[i]}`
      tagList.push(x.tags[i])
    }
  }
})

let checkboxes = document.querySelector('.checkbox')
checkboxes.addEventListener("click", searchTag)
function searchTag() {
  inputSearch.value = ''
  dropdown.value = ''
  let markedCheckbox = document.querySelectorAll('input[type="checkbox"]:checked');
  let newGlossary = []
  for (let cb of markedCheckbox) {
    newGlossary = [...newGlossary, ...(glossary.filter(x => x.tags.indexOf(cb.value) >= 0))]
  }

  table.innerHTML = ''
  firstTR.innerHTML = ''
  resetTable(newGlossary)
}

function resetTable(lst) {
  table.appendChild(firstTR)
  firstTR.innerHTML = '<tr><th>Term</th><th>Class</th><th>Definition</th><th>Tags</th></tr>'
  lst.forEach(x => {
    let tr = document.createElement("tr")
    table.appendChild(tr)
    tr.innerHTML = `<td>${x.term}</td><td>${x.class}</td><td>${x.definition}</td><td>${x.tags}</td>`
  })

  let allTD = document.querySelectorAll("table td")
  let allTH = document.querySelectorAll("table th")
  allTD.forEach(x => x.style.border = "1px solid")
  allTH.forEach(x => x.style.border = "1px solid")
}
